...
def eval_track_one_epoch(model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None):
    ...
    for f_index in range(len(first_index)-1):
        ...
        if length > 0:
            for index in range(st+1, ov):
                ...
                # with open('Success_main.txt', 'w') as f:
                #     f.write(str(Success_main.average.item()) + '\n')
                # with open('Success_main_2d.txt', 'a') as f:
                #     f.write(str(Success_main) + '\n')
                # with open('Precision_main.txt', 'w') as f:
            ...
            with open('Success_main.value.txt', 'a') as f:
