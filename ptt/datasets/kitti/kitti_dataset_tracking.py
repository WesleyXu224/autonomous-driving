...
class KittiTrackingDataset(DatasetTemplate):
    ...
    def get_tracklet_list(self, sceneID):
        ...
        sceneID = ["testing" ,"training"]
        print(f"lidar_path:{lidar_path}")
        list_of_scene = [
            path for path in os.listdir(lidar_path)
            if os.path.isdir(os.path.join(lidar_path, path)) and path in sceneID
        ]
        # list_of_scene = [path for path in os.listdir(lidar_path)]
        print(sceneID)
        # 打印每个path的值
        # for path in list_of_scene:
        #     print(f"path: {path}")
